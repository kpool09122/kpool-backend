...
マージ完了後、使用済みのworktreeとworktreeブランチを削除してください。

```bash
# worktreeを削除
git worktree remove --force .claude/worktrees/{worktree名}

# worktreeブランチを削除
git branch -D {worktreeブランチ名}
```

**重要**: このクリーンアップを忘れると、worktreeが残り続けてブランチ削除ができなくなります。必ずマージ後に実行してください。
...
- **受け入れ条件**: `task check` が通ること + Codexレビューで致命的な指摘がないこと