...
class TalentTest extends TestCase
{
    ...
    public function test__construct(): void
    {
        ...
        $this->assertSame($talentInfo->normalizedName, $talentInfo->talent->normalizedName());
        $this->assertSame((string)$talentInfo->realName, (string)$talentInfo->talent->realName());
        $this->assertSame($talentInfo->normalizedRealName, $talentInfo->talent->normalizedRealName());
    ...
    public function testSetName(): void
    {
        ...
        $talentInfo->talent->setName($newName);
        $this->assertNotSame((string)$talentInfo->name, (string)$talentInfo->talent->name());
        $this->assertSame((string)$newName, (string)$talentInfo->talent->name());
    }
    ...
    public function testSetNormalizedName(): void
    {
        ...
        $talentInfo->talent->setNormalizedName($newNormalizedName);
        $this->assertNotSame($talentInfo->normalizedName, $talentInfo->talent->normalizedName());
        $this->assertSame($newNormalizedName, $talentInfo->talent->normalizedName());
    }
    ...
    public function testSetRealName(): void
    {
        ...
        $talentInfo->talent->setRealName($newRealName);
        $this->assertNotSame((string)$talentInfo->realName, (string)$talentInfo->talent->realName());
        $this->assertSame((string)$newRealName, (string)$talentInfo->talent->realName());
    }
    ...
    public function testSetNormalizedRealName(): void
    {
        ...
        $talentInfo->talent->setNormalizedRealName($newNormalizedRealName);
        $this->assertNotSame($talentInfo->normalizedRealName, $talentInfo->talent->normalizedRealName());
        $this->assertSame($newNormalizedRealName, $talentInfo->talent->normalizedRealName());
    }
    ...
    public function testSetGroupIdentifier(): void
    {
        ...
        $talentInfo->talent->setGroupIdentifiers($newGroupIdentifiers);
        $this->assertNotSame($talentInfo->groupIdentifiers, $talentInfo->talent->groupIdentifiers());
        $this->assertSame($newGroupIdentifiers, $talentInfo->talent->groupIdentifiers());

        $talentInfo->talent->setGroupIdentifiers([]);
        $this->assertEmpty($talentInfo->talent->groupIdentifiers());
    }
    ...
    public function testSetBirthday(): void
    {
        ...
        $talentInfo->talent->setBirthday($newBirthday);
        $this->assertNotSame((string)$talentInfo->birthday, (string)$talentInfo->talent->birthday());
        $this->assertSame((string)$newBirthday, (string)$talentInfo->talent->birthday());
    }
...
我希望能活用至今在職涯中所累積的「精準掌握客戶問題的能力」以及「以數據為基礎制定並執行策略的能力」，為貴公司的業務成長做出貢獻。我相信未來能憑藉我同時具備業務與行銷的雙重觀點此一優勢，實現更有效的客戶開發與接觸。');
        ...
        $talentInfo->talent->setCareer($newCareer);
        $this->assertNotSame((string)$talentInfo->career, (string)$talentInfo->talent->career());
        $this->assertSame((string)$newCareer, (string)$talentInfo->talent->career());
    }
    ...
    public function testSetImageLink(): void
    {
        ...
        $talentInfo->talent->setImageLink($newImagePath);
        $this->assertNotSame((string)$talentInfo->imagePath, (string)$talentInfo->talent->imageLink());
        $this->assertSame((string)$newImagePath, (string)$talentInfo->talent->imageLink());
    }
