...
class GroupTest extends TestCase
{
    ...
    public function test__construct(): void
    {
        ...
        $this->assertSame((string)$createGroup->editorIdentifier, (string)$group->editorIdentifier());
        $this->assertSame((string) $createGroup->approverIdentifier, (string)$group->approverIdentifier());
...
她們從出道初期的活潑可愛風格，隨著逐年成長，展現出從成熟帥氣到洗練的表演等多樣面貌。其特色是琅琅上口的旋律和易於模仿的舞蹈，獲得了各年齡層的支持。不僅在韓國和日本，她們也成功舉辦了世界級的體育場巡迴演唱會等，作為代表K-POP的頂級團體，至今仍然持續吸引著全世界的粉絲。官方粉絲名稱為「ONCE」。');
    ...
    public function testSetSourceEditorIdentifier(): void
    {
        ...
        // 初期値はnull
        $this->assertNull($group->sourceEditorIdentifier());

        // 値を設定
        $sourceEditorIdentifier = new PrincipalIdentifier(StrTestHelper::generateUuid());
        $group->setSourceEditorIdentifier($sourceEditorIdentifier);
        $this->assertSame($sourceEditorIdentifier, $group->sourceEditorIdentifier());

        // nullを設定
        $group->setSourceEditorIdentifier(null);
        $this->assertNull($group->sourceEditorIdentifier());
    }
    ...
    public function testSetTranslatedAt(): void
    {
        ...
        // 初期値はnull
        $this->assertNull($group->translatedAt());

        // 値を設定
        $translatedAt = new DateTimeImmutable('2026-01-02 12:00:00');
        $group->setTranslatedAt($translatedAt);
        $this->assertSame($translatedAt, $group->translatedAt());

        // nullを設定
        $group->setTranslatedAt(null);
        $this->assertNull($group->translatedAt());
    }
    ...
    public function testSetApprovedAt(): void
    {
        ...
        // 初期値はnull
        $this->assertNull($group->approvedAt());

        // 値を設定
        $approvedAt = new DateTimeImmutable('2026-01-02 12:00:00');
        $group->setApprovedAt($approvedAt);
        $this->assertSame($approvedAt, $group->approvedAt());

        // nullを設定
        $group->setApprovedAt(null);
        $this->assertNull($group->approvedAt());
    }
