...
namespace Tests\Wiki\Agency\Application\UseCase\Command\SubmitUpdatedAgency;
...
use Source\Wiki\Agency\Application\UseCase\Command\SubmitUpdatedAgency\SubmitUpdatedAgencyInput;
...
class SubmitUpdatedAgencyTest extends TestCase
{
    /**
     * 正常系: インスタンスが生成されること
     *
...
등 세계적인 인기를 자랑하는 그룹이 다수 소속되어 있으며, K팝의 글로벌한 발전에서 중심적인 역할을 계속해서 맡고 있습니다. 음악 사업 외에 배우 매니지먼트나 공연 사업도 하고 있습니다.');
        $input = new SubmitUpdatedAgencyInput(
            $agencyIdentifier,
        );
        ...
        $this->app->instance(AgencyRepositoryInterface::class, $agencyRepository);
        $submitUpdatedAgency = $this->app->make(SubmitUpdatedAgencyInterface::class);
        $agency = $submitUpdatedAgency->process($input);
    ...
    public function testWhenNotFoundAgency(): void
    {
        ...
        $submitUpdatedAgency = $this->app->make(SubmitUpdatedAgencyInterface::class);
        $submitUpdatedAgency->process($input);
    }
