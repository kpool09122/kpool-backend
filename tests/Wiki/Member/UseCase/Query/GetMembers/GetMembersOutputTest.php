...
use Businesses\Wiki\Member\UseCase\Query\GetMembers\GetMembersOutput;
use Businesses\Wiki\Member\UseCase\Query\MemberReadModel;
use Businesses\Wiki\Member\UseCase\Query\SongReadModel;
use DateTimeImmutable;
use Tests\Helper\StrTestHelper;
use Tests\TestCase;

class GetMembersOutputTest extends TestCase
{
    /**
    ...
    public function testOutput(): void
    {
        ...
        $readModel1 = new MemberReadModel(
            StrTestHelper::generateUlid(),
            '사나',
            '미나토자키 사나',
            ['TWICE', 'MISAMO'],
        ...
        $readModel2 = new MemberReadModel(
            StrTestHelper::generateUlid(),
            '지효',
            '박지효',
            ['TWICE'],
        ...
        $members = [$readModel1, $readModel2];
        $currentPage = 1;
        $lastPage = 2;
        $total = 3;
        $output = new GetMembersOutput();
        $output->output(
            $members,
            $currentPage,
            $lastPage,
        ...
        $this->assertSame([
            'members' => [$readModel1->toArray(), $readModel2->toArray()],
