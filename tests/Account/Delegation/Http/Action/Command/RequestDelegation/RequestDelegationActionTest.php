...
namespace Tests\Account\Delegation\Http\Action\Command\RequestDelegation;
...
class RequestDelegationActionTest extends TestCase
{
    ...
    public function testReturnsCreatedWithAccountLevelResponse(): void
    {
        ...
        /** @var RequestDelegationInterface&Mockery\MockInterface $useCase */
        $useCase = Mockery::mock(RequestDelegationInterface::class);
        $useCase->shouldReceive('process')->once()->with(
            Mockery::on(fn ($input) => $input instanceof RequestDelegationInput
                && $input->principal() === $principal
                && (string) $input->targetAccountIdentifier() === (string) $target),
            Mockery::on(function ($output) use ($delegation): bool {
                if (! $output instanceof RequestDelegationOutput) {
                    return false;
                }
